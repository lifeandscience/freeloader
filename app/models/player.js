...
PlayerSchema.methods.notifyOfNewGroupDueToAbandonment = function(callback) {
	var url = process.env.BASEURL + '/play';
	var title = 'Yesterday your old group disbanded.';
	var text = ' The group you were in grew too small, so your old group was disbanded. You are now in a new group. Make a decision for today at\n\n'+url;
	this.notify("info", ['web', 'email'], title, text, callback);
};
PlayerSchema.methods.notifyOfNewGroupDueToWalkaway = function(callback) {
	var url = process.env.BASEURL + '/play';
	var title = 'Yesterday you walked away from your old group.';
	var text = ' You chose to leave your old group yesterday. So, you were placed in a new group. Make a decision for today at\n\n'+url;
	this.notify("info", ['web', 'email'], title, text, callback);
};
PlayerSchema.methods.notifyOfNewGroupDueToNewbie = function(callback) {
	var url = process.env.BASEURL + '/play';
	var title = 'You have been assigned to a group. Play now!';
	var text = ' You were placed in a new group. Make a decision for today at\n\n'+url;
	this.notify("info", ['web', 'email'], title, text, callback);
};
PlayerSchema.methods.notifyOfDesertion = function(callback) {
	var title = 'Yesterday you left your spot in Freeloader.';
	var text = ' You were unenrolled from Freeloader yesterday. Your balance has been reset to zero and you\'ll no longer be prompted to play unless you re-enroll.';
	this.notify("info", ['web', 'email'], title, text, callback);
};
